...
unsigned char DES_BITS_CONJUCTIONS[] = {
...
unsigned char SHIFTS_CONJUCTIONS[] = {
	// Jeden bit, dwa bity
	128, 192,
...
void DES_start(des_t* self) {
 ...
 // Krok 1
void DES_initial_permutation(des_t* self) {
	int ip_index;
	int ip_byte;
	int ip_bit;
...
void DES_shift_key56(des_t* self) {
...
void DES_key56_to_48(des_t* self) {
	unsigned char current_bit_value;
	// Czyszczenie śmieci
	for (int i = 0; i < 6; i++) {
		self->_key48[i] = 0;
...
void DES_final_permutation(des_t* self) {
	for (int i = 0; i < self->encrypted_text_length; i++) {
		self->_tmp_storage[i] = self->encrypted_text[i];
		self->encrypted_text[i] = 0;
	}
